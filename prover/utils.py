...
from mytransformers import AutoTokenizer, T5ForConditionalGeneration, T5Tokenizer, AutoConfig, AutoModel
...
class EBRLMetric(Metric):
    ...
    def update(self, loss, pred, target, reward):
        hits = 0
        total_reward = 0
        all_correct = 1
        ...
        for i, pred_step in enumerate(pred):
            ...
            # if i < len(gold) and set(gold_step) == set(pred_step):
            if set(gold_step) == set(aligned_pred_step):
                hits += 1
                # total_reward += 1 * (self.discount ** i)
                total_reward += 1
                # if set(gold_step) != set(pred_step):
                #     all_correct = 0
            else:
                # total_reward -= 1 * (self.discount ** i)
                total_reward -= 1
        ...
        if all_correct == 1:
            ...
            if len(target) == 1:
                ...
                self.len_5 += 1
            if self.verbose and len(target) > 1:
                print(pred)
...
class ScheduledSamplingCallback(Callback):
    def __init__(self, mode='linear'):
        super().__init__()
        self.mode = mode
        self.linear_lb = 0.5  # linear lower bound
...
class ParaPattern(object):
    def __init__(self, device='cuda:1', inter_cache_path=None):
        ...
        self.model = mytransformers.AutoModelForSeq2SeqLM.from_pretrained(self.model_path)
        self.tokenizer = mytransformers.AutoTokenizer.from_pretrained(self.model_path, use_fast=False)
    ...
    def get_intermediate_text(self, sent1, sent2):
        ...
        out_sent = decapitalize(decoded[0])
        conclusion = out_sent.replace('"', '').replace('\'', '').strip()
