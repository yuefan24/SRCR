...
import sys
from mytransformers import get_cosine_schedule_with_warmup
from typing import *
...
def deserialize(
    ...
    if 'hypothesis' in proof:
        return nodes.get("hypothesis", None)
    else:
        proof_step_last = proof.split(";")[-1]
        premises, conclusion = proof_step_last.split(" -> ")
        m = re.fullmatch(r"\((?P<score>.+?)\) (?P<concl>.+)", conclusion)
        score: Optional[str]
        if m is not None:
            score = m["score"]
