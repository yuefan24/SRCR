...
class EntailmentWriter(pl.LightningModule):
    def __init__(
        ...
        model_name_base = 'modal_path/T5-large'
    ...
    def generate_greedy_proofs(
        ...
        for _ in range(self.max_num_steps):
            ...
            proof_steps = [
                steps[max_s[i]] if len(steps) > 0 else None for i, steps in enumerate(proof_steps)           # 选得分最高的
            ]
    ...
    def training_step(self, batch: Batch, batch_idx: int) -> Dict[str, torch.Tensor]:  # type: ignore
        if self.stepwise:
            self.num_step += 1
            loss = self(
                batch["input_seq_ids"], batch["input_seq_mask"], batch["output_seq_ids"], batch["sentence_span_list"], batch["sentence_current_label"]
            )
